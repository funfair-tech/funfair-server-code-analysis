...
public sealed class ProhibitedMethodInvocationsDiagnosticsAnalyzer : DiagnosticAnalyzer
{
    ...
    private sealed class Checker
    {
        ...
        private void LookForBannedInvocation(
        ...
        )
        {
            ...
            IReadOnlyDictionary<ProhibitedMethodsSpec, IReadOnlyList<IMethodSymbol>> cachedSymbols =
                this.LoadCachedSymbols(compilation);

            Mapping mapping = new(methodName: memberSymbol.Name, className: fullyQualifiedName);

            foreach (
                ProhibitedMethodsSpec prohibitedMethod in BannedMethods.Where(predicate: rule =>
            ...
            )
            {
                if (
                    !cachedSymbols.TryGetValue(
                        key: prohibitedMethod,
                        out IReadOnlyList<IMethodSymbol> prohibitedMethodSignatures
                    )
                )
                {
                    continue;
                ...
                if (
                    IsBannedMethodSignature(
                        invocationArguments: memberSymbol,
                        methodSignatures: prohibitedMethodSignatures
                    )
                )
                {
        ...
        )
        {
            ...
            foreach (ProhibitedMethodsSpec rule in BannedMethods)
            {
                ...
                IReadOnlyList<IMethodSymbol> methodSignatures = GetOverloads(
                ...
                if (methodSignatures.Count != 0)
                {
                    cachedSymbols.Add(
                        key: rule,
                        RemoveAllowedSignaturesForMethod(
                            methodSignatures: methodSignatures,
                            ruleSignatures: rule.BannedSignatures
                        )
                    );
                }
            }
        ...
        private static IReadOnlyList<IMethodSymbol> GetOverloads(
            INamedTypeSymbol sourceClassType,
            ProhibitedMethodsSpec rule
        )
        {
            return
            [
        ...
        private static IReadOnlyList<IMethodSymbol> RemoveAllowedSignaturesForMethod(
            IReadOnlyList<IMethodSymbol>? methodSignatures,
        ...
        private static IReadOnlyList<IMethodSymbol> BuildMethodSignatureList(
            IEnumerable<IEnumerable<string>> ruleSignatures,
            IReadOnlyList<IMethodSymbol> methodSymbols
        )
        {
            return
            [
                .. ruleSignatures.SelectMany(ruleSignature =>
                    methodSymbols.Where(methodSymbol =>
                        methodSymbol
                            .Parameters.Select(selector: parameterSymbol =>
        ...
        )
        {
            ...
            return methodSignatures.Any(predicate: methodSignature =>
                methodSignature
                    .Parameters.Select(x => SymbolDisplay.ToDisplayString(x.OriginalDefinition))
        ...
        private static IReadOnlyList<string> GetInvocationParameters(IMethodSymbol invocationArguments)
        {
            return
            [
                .. invocationArguments.Parameters.Select(parameter =>
                    SymbolDisplay.ToDisplayString(parameter.OriginalDefinition)
                ),
            ];
        }
    }
    ...
    private readonly record struct ProhibitedMethodsSpec
    {
        ...
        )
        {
            ...
            this.Rule = RuleHelpers.CreateRule(
                code: ruleId,
                category: Categories.IllegalMethodInvocations,
                title: title,
                message: message
            );
